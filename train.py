...
from script.util import reconstruct_R, tensor_to_vae_latent, load_config, dump_config
from script.resnet import ResNet
...
def parse_args():
    ...
    parser.add_argument(
        "--base_folder",
        default='./existed_stable_video_diffusion',
        required=False,
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config",
        type=str,
        default='/data/xu_23/Project/nvs_solver/experiments.svd/2026011301_TrainingOnRealData/train@hs-ergb_sub@train.yaml',
    ...
    assert len(config.output_folder) > 0
    # if os.path.exists(config.output_folder):
    #     shutil.rmtree(config.output_folder)
    os.makedirs(config.output_folder, exist_ok=True)
    # if os.path.exists(config.parameter_folder):
    #     shutil.rmtree(config.parameter_folder)
    os.makedirs(config.parameter_folder, exist_ok=True)
    ...
    dump_config(os.path.join(config.log_folder, 'config.yaml'), config)
...
def main():
    ...
    accelerator_project_config = ProjectConfiguration(
        project_dir=config.output_folder, logging_dir=config.log_folder)
    ...
    for network in config.network:
        networks[network['name']].to(accelerator.device, dtype=weight_dtype)
    ...
    train_dataset = EventAndVideoDataset(
        folder_path=config.data.train_folder_path,
        mode='train',
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = os.listdir(config.parameter_folder)
        ...
        if path is None:
            ...
            accelerator.load_state(os.path.join(config.parameter_folder, path))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                assert batch["rgb_raw"].shape[0] == 1 and batch["rgb_raw"].shape[1] - batch["event"].shape[1] == 1
                rgb_values = batch["rgb_raw"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                ) # B, 13, C, H, W
                event_values = batch["event"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                ) # B, 12, C, H, W
                condition_mode = batch['condition_mode'][0]
                # 
                conditional_event_values = event_values # [-N~+N]
                if condition_mode == 'R-E-R':
                    encoder_hidden_pixel_values = rgb_values[:, 0, :, :, :]
                    conditional_reference_values = torch.cat([rgb_values[:, 0:1, :, :, :], rgb_values[:, -1:, :, :, :]], dim=1)
                    pixel_values = torch.cat([rgb_values, torch.zeros_like(rgb_values[:, 0:1, :, :, :])], dim=1)
                elif condition_mode == 'R-E-N':
                    encoder_hidden_pixel_values = rgb_values[:, 0, :, :, :]
                    conditional_reference_values = torch.cat([rgb_values[:, 0:1, :, :, :], torch.zeros_like(rgb_values[:, -1:, :, :, :])], dim=1)
                    pixel_values = torch.cat([rgb_values, torch.zeros_like(rgb_values[:, 0:1, :, :, :])], dim=1)
                elif condition_mode == 'N-E-R':
                    encoder_hidden_pixel_values = rgb_values[:, -1, :, :, :]
                    conditional_reference_values = torch.cat([torch.zeros_like(rgb_values[:, 0:1, :, :, :]), rgb_values[:, -1:, :, :, :]], dim=1)
                    pixel_values = torch.cat([rgb_values, torch.zeros_like(rgb_values[:, 0:1, :, :, :])], dim=1)
                elif condition_mode == 'N-E-N':
                    encoder_hidden_pixel_values = event_values[:, 0, :, :, :]
                    conditional_reference_values = None # torch.cat([torch.zeros_like(rgb_values[:, 0:1, :, :, :]), torch.zeros_like(rgb_values[:, -1:, :, :, :])], dim=1)
                    pixel_values = rgb_values[:, 1:, :, :, :] # B, 13?, C, H, W
                else:
                    raise ValueError
                
                latents = tensor_to_vae_latent(pixel_values, networks['vae_rgb'])
                ...
                noise_aug_strength = cond_sigmas[0] # TODO: support batch > 1 ######################################################################### WARNING
                cond_sigmas = cond_sigmas[:, None, None, None, None]
                # 
                
                conditional_event_values = torch.randn_like(conditional_event_values) * cond_sigmas + conditional_event_values
                conditional_event_latents = tensor_to_vae_latent(conditional_event_values, networks['vae_event'])
                conditional_event_latents = conditional_event_latents / networks['vae_event'].config.scaling_factor
                if condition_mode == 'N-E-N':
                    conditional_latents = conditional_event_latents
                else:
                    conditional_reference_values = torch.randn_like(conditional_reference_values) * cond_sigmas + conditional_reference_values
                    conditional_reference_latents = tensor_to_vae_latent(conditional_reference_values, networks['vae_rgb'])
                    conditional_reference_latents = conditional_reference_latents / networks['vae_rgb'].config.scaling_factor
                    conditional_latents = torch.cat([conditional_reference_latents, conditional_event_latents], dim=1) # tensor_to_vae_latent(conditional_pixel_values, vae)
                ...
                assert condition_mode == 'N-E-N'

                ####################################
                B, F, C, H, W = latents.shape
                rgb_latents = latents
                event_latents = networks['vae_event'].config.scaling_factor * conditional_event_latents

                if config.train.pipeline['name'] == 'default':
                    noisy_latents = rgb_latents + noise * sigmas
                    target_latents = rgb_latents
                else:
                    raise ValueError
                ####################################
                ...
                encoder_hidden_states = encode_image(
                    encoder_hidden_pixel_values.float())
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    if len(conditional_latents.shape) == 4:
                ...
                loss = 0
                for criterion in config.train.criterion:
                    if criterion['name'] == 'l2_loss':
                        # MSE loss
                        loss_value = torch.mean(
                            (weighing.float() * (denoised_latents.float() -
                            target_latents.float()) ** 2).reshape(target_latents.shape[0], -1),
                            dim=1,
                        )
                        loss_value = loss_value.mean()
                    elif criterion['name'] == 'latent_space_event_prediction_loss':
                        start_rgb_latents = denoised_latents[:, :-1, :, :, :]
                        end_rgb_latents = denoised_latents[:, 1:, :, :, :]
                        coupled_rgb_latents = torch.cat([start_rgb_latents, end_rgb_latents], dim=2)
                        frame_num = coupled_rgb_latents.shape[1]
                        coupled_rgb_latents = rearrange(coupled_rgb_latents, "b f c h w -> (b f) c h w")
                        if next(iter(networks['resnet'].parameters())).dtype == torch.float16:
                            networks['resnet'] = networks['resnet'].to(dtype=torch.float32)
                        pred_event_latents = networks['resnet'](coupled_rgb_latents)
                        pred_event_latents = rearrange(pred_event_latents, "(b f) c h w -> b f c h w", f=frame_num)
                        gt_event_latents = event_latents[:, 1:, :, :, :]
                        loss_value = torch.mean(torch.pow(pred_event_latents - gt_event_latents, 2))

                    loss += loss_value * criterion['lambda']
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.checkpoints_total_limit is not None:
                            checkpoints = os.listdir(config.parameter_folder)
                            checkpoints = [
                            ...
                            if len(checkpoints) >= args.checkpoints_total_limit:
                                ...
                                for removing_checkpoint in removing_checkpoints:
                                    removing_checkpoint = os.path.join(
                                        config.parameter_folder, removing_checkpoint)
                                    shutil.rmtree(removing_checkpoint)

                        save_path = os.path.join(config.parameter_folder, f"checkpoint-{global_step:07d}")
                        accelerator.save_state(save_path)

                        params_with_grad = OrderedDict()
                        # 
                        ...
                        for name, param in named_parameters:
                            if param.requires_grad == True:
                                params_with_grad[name] = state_dict[name]
                        torch.save(params_with_grad, os.path.join(config.parameter_folder, f'unet_train_params-{global_step:07d}.pth'))
    ...
    if accelerator.is_main_process:
        ...
        pipeline.save_pretrained(config.parameter_folder)

        if args.push_to_hub:
            upload_folder(
                repo_id=repo_id,
                folder_path=config.log_folder,
